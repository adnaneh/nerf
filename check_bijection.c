...
static const uint8_t confusion[512] = {
...
int main() {
    // Check S_low
    int low_seen[256] = {0};
    int low_duplicates = 0;
    for (int i = 0; i < 256; i++) {
        if (low_seen[confusion[i]]++) {
            low_duplicates++;
        }
    }
    ...
    // Check S_high
    int high_seen[256] = {0};
    int high_duplicates = 0;
    for (int i = 0; i < 256; i++) {
        if (high_seen[confusion[256+i]]++) {
            high_duplicates++;
        }
    }
    
    printf("S_low duplicates: %d\n", low_duplicates);
    printf("S_high duplicates: %d\n", high_duplicates);
    
    // Count missing values
    int low_missing = 0, high_missing = 0;
    for (int i = 0; i < 256; i++) {
        if (low_seen[i] == 0) low_missing++;
        if (high_seen[i] == 0) high_missing++;
    }
    printf("S_low missing values: %d\n", low_missing);
    printf("S_high missing values: %d\n", high_missing);
