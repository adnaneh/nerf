...
static int dfs_recursive(u8 state[32], int round, u8 solution[32])
{
    ...
    if (!valid_state) {
        ...
        if (combo_log) {
            fprintf(combo_log, "%d,%d,0,0,0,0,0\n", current_run_id, round);
        }
        return 0;
    }
    ...
    for (int j = 0; j < 32; ++j) {
        ...
        if (total_combinations > MAX_COMBO_LIMIT) {
            // Log early exit with v analysis
            if (combo_log) {
                int v_sum = 0;
                int v_xor = 0;
                int multi_choice_count = 0;
                for (int k = 0; k < 32; k++) {
                    v_sum += v[k];
                    v_xor ^= v[k];
                    if (inv_low_count[v[k]] > 1) multi_choice_count++;
                }
                fprintf(combo_log, "%d,%d,%lld,0,%d,%d,%d\n", 
    ...
    for (int combo_offset = 0; combo_offset < total_combinations; ++combo_offset) {
        ...
        if (result) {
            ...
            if (combo_log) {
                ...
                fprintf(combo_log, "%d,%d,%lld,1,%d,%d,%d\n", 
                        current_run_id, round, total_combinations, v_sum, v_xor, multi_choice_count);
            }
            recursion_depth--;
    ...
    // Log failed path after trying all combinations with v analysis
    if (combo_log) {
        int v_sum = 0;
        int v_xor = 0;
        int multi_choice_count = 0;
...
static int inverse_256_rounds_dfs(u8 solution[32], const u8 initial_state[32])
{
    ...
    if (!combo_log) {
        ...
        fprintf(combo_log, "run_id,round,total_combinations,success,v_sum,v_xor,multi_choice_count\n");
    }
    current_run_id++;
