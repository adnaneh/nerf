// -----------------------------------------------------------------------------
//  "Hire‑me" crack‑me – OPTIMIZED Level‑2 solver
// -----------------------------------------------------------------------------
//  Implements optimizations 1-6 from the provided list
// -----------------------------------------------------------------------------
...
// Optimized dot_row using builtin
...
// OPTIMIZATION 5: Modified structure with count first for cache-friendly access
...
// OPTIMIZATION 6: Deterministic random function
static inline uint32_t deterministic_random(uint32_t state)
{
    return state * 0x9E3779B9u;
}

static int dfs_recursive(u8 state[32], int round, u8 solution[32], uint32_t seed)
{
    call_count++;
    ...
    // OPTIMIZATION 5: Cache pointers to inv_low arrays
    ...
    // OPTIMIZATION 2: Calculate total with early bailout
    long long total_combinations = 1;
    for (int j = 0; j < 32; ++j) {
        int c = choices_per_pos[j];
        if (c == 0) return 0;  // Impossible
    ...
    uint32_t combo_start = deterministic_random(seed + round) % (uint32_t)total_combinations;
    
    // Try combinations
    for (int combo_offset = 0; combo_offset < total_combinations; ++combo_offset) {
        uint32_t combo = (combo_start + combo_offset) % (uint32_t)total_combinations;
        
        // Generate state for this combination
        // Key insight: we must generate based on original order, not sorted order
        uint32_t temp_combo = combo;
        for (int j = 0; j < 32; ++j) {
            int choice_idx = temp_combo % choices_per_pos[j];
            temp_combo /= choices_per_pos[j];
            new_state[j] = choices_ptr[j][choice_idx];
        }
        
        int result = dfs_recursive(new_state, round + 1, solution, seed);
        if (result) {
            recursion_depth--;
            return 1;
        }
...
static void build_final_state(u8 c[32], uint32_t seed)
{
    ...
    for (int attempt = 0; attempt < max_attempts; ++attempt) {
        ...
        uint32_t rng = deterministic_random(seed + attempt);
        
        int valid_state = 1;
        for (int pos = 0; pos < 16 && valid_state; ++pos) {
            int found = 0;
            uint32_t start = rng % 256;
            rng = deterministic_random(rng);
            
            for (int tries = 0; tries < 256; ++tries) {
                int ev = (start + tries) % 256;
...
int main(void)
{
    ...
    printf("Running optimized solver with all 6 optimizations...\n");
    
    for (int attempt = 0; attempt < 10000; ++attempt) {
        u8 c256[32];
        build_final_state(c256, seed + attempt * 1000);
        ...
        int found = dfs_recursive(c256, 0, solution, seed + attempt);
        ...
        if (found) {
            ...
            if (!memcmp(test_out, target, 16)) {
                ...
                printf("  Solution found in %.2f ms (%d calls)\n", solve_time, call_count);
    ...
    fprintf(stderr, "[FATAL] Could not find solution\n");
    return 1;
}