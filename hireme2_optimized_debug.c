...
static int dfs_recursive_original(u8 state[32], int round, u8 solution[32])
{
    if (round == 256) {
    ...
    for (int combo_offset = 0; combo_offset < total_combinations; ++combo_offset) {
        ...
        for (int j = 0; j < 32; ++j) {
            ...
            new_state[j] = inv_low_cache[v[j]].values[choice_idx];
        }
        
        int result = dfs_recursive_original(new_state, round + 1, solution);
        if (result) {
            return 1;
        }
    }
...
int main(void)
{
    ...
    printf("Testing with simpler DFS (no sorting)...\n");
    ...
    for (int attempt = 0; attempt < 1000; ++attempt) {
        ...
        u8 solution[32];
        int found = dfs_recursive_original(c256, 0, solution);
