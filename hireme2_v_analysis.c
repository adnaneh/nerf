...
// V-vector analysis logging
static FILE* v_analysis_log = NULL;
...
static int dfs_recursive(u8 state[32], int round, u8 solution[32])
{
    ...
    if (!valid_state) {
        // Log failed path - no valid pre-images
        if (v_analysis_log) {
            fprintf(v_analysis_log, "%d,%d,0,0,0,0,0\n", current_run_id, round);
        }
        return 0;
    }
    ...
    for (int combo_offset = 0; combo_offset < total_combinations; ++combo_offset) {
        ...
        for (int j = 0; j < 32; ++j) {
            ...
            // Log individual choices for multi-choice outputs
        ...
        if (result) {
            // Log successful path with v analysis
            if (v_analysis_log) {
                int v_sum = 0, v_xor = 0, multi_choice_count = 0;
                for (int k = 0; k < 32; k++) {
                    v_sum += v[k];
                    v_xor ^= v[k];
                    if (inv_low_count[v[k]] > 1) multi_choice_count++;
                }
                fprintf(v_analysis_log, "%d,%d,%lld,1,%d,%d,%d\n", 
                        current_run_id, round, total_combinations, v_sum, v_xor, multi_choice_count);
            }
            recursion_depth--;
    ...
    // Log failed path after trying all combinations
    if (v_analysis_log) {
        int v_sum = 0, v_xor = 0, multi_choice_count = 0;
        for (int k = 0; k < 32; k++) {
            v_sum += v[k];
        ...
        fprintf(v_analysis_log, "%d,%d,%lld,0,%d,%d,%d\n", 
                current_run_id, round, total_combinations, v_sum, v_xor, multi_choice_count);
    }
...
static int inverse_256_rounds_dfs(u8 solution[32], const u8 initial_state[32])
{
    ...
    // Initialize v-analysis logging
    if (!v_analysis_log) {
        v_analysis_log = fopen("v_analysis.csv", "w");
        fprintf(v_analysis_log, "run_id,round,total_combinations,success,v_sum,v_xor,multi_choice_count\n");
    }
    current_run_id++;
