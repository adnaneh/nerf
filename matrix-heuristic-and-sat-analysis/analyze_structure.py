#!/usr/bin/env python3

# The confusion array from the C code
confusion = [
    
]

# The diffusion matrix from the C code
diffusion = [
    0xf26cb481,0x16a5dc92,0x3c5ba924,0x79b65248,0x2fc64b18,0x615acd29,0xc3b59a42,0x976b2584,
    0x6cf281b4,0xa51692dc,0x5b3c24a9,0xb6794852,0xc62f184b,0x5a6129cd,0xb5c3429a,0x6b978425,
    0xb481f26c,0xdc9216a5,0xa9243c5b,0x524879b6,0x4b182fc6,0xcd29615a,0x9a42c3b5,0x2584976b,
    0x81b46cf2,0x92dca516,0x24a95b3c,0x4852b679,0x184bc62f,0x29cd5a61,0x429ab5c3,0x84256b97
]

def create_inverse_confusion():
    """Create inverse lookup table for the first half of confusion matrix"""
    inv_confusion1 = [0] * 256
    for i in range(256):
        inv_confusion1[confusion[i]] = i
    return inv_confusion1

def invert_diffusion_matrix_gf2(original):
    """Invert the diffusion matrix in GF(2)"""
    n = 32
    # Create identity matrix for inverse
    inverse = [1 << i for i in range(n)]
    
    # Copy original to work matrix
    work = original.copy()
    
    # Gaussian elimination in GF(2)
    for col in range(n):
        # Find pivot
        pivot = -1
        for row in range(col, n):
            if work[row] & (1 << col):
                pivot = row
                break
        
        if pivot == -1:
            continue  # Singular matrix
        
        # Swap rows if needed
        if pivot != col:
            work[col], work[pivot] = work[pivot], work[col]
            inverse[col], inverse[pivot] = inverse[pivot], inverse[col]
        
        # Eliminate
        for row in range(n):
            if row != col and (work[row] & (1 << col)):
                work[row] ^= work[col]
                inverse[row] ^= inverse[col]
    
    return inverse

def visualize_matrix_ascii(matrix, name, size=32):
    """Create ASCII visualization of a binary matrix"""
    print(f"\n{name} (showing density with # for 1, . for 0):")
    print("     " + "".join(str(i % 10) for i in range(size)))
    for i in range(size):
        row_str = f"{i:3d}: "
        for j in range(size):
            bit = (matrix[i] >> j) & 1
            row_str += "#" if bit else "."
        print(row_str)

def analyze_structure():
    """Analyze the structure of inv_confusion1 * inv_diffusion"""
    print("="*60)
    print("Structure Analysis: inv_confusion1 * inv_diffusion")
    print("="*60)
    
    # Create inverse confusion
    inv_confusion1 = create_inverse_confusion()
    print(f"\n1. Inverse Confusion (inv_confusion1):")
    print(f"   - Type: Permutation (bijective mapping)")
    print(f"   - Size: 256 → 256")
    print(f"   - Operation: Byte substitution")
    print(f"   - First 10 mappings: ", end="")
    for i in range(10):
        print(f"{i}→{inv_confusion1[i]}", end=" ")
    print()
    
    # Create inverse diffusion
    diffusion_inv = invert_diffusion_matrix_gf2(diffusion)
    print(f"\n2. Inverse Diffusion Matrix:")
    print(f"   - Type: Binary matrix in GF(2)")
    print(f"   - Size: 32×32")
    print(f"   - Operation: Linear transformation (XOR-based)")
    
    # Count 1s in diffusion matrix
    total_ones = sum(bin(row).count('1') for row in diffusion_inv)
    density = total_ones / (32 * 32)
    print(f"   - Density: {total_ones}/{32*32} = {density:.2%}")
    
    # Show the matrices
    visualize_matrix_ascii(diffusion_inv, "Inverse Diffusion Matrix")
    
    print("\n3. Combined Operation (inv_confusion1 * inv_diffusion):")
    print("   The actual operation in the Backward() function:")
    print("   a) Apply inverse diffusion: temp[j] = XOR of (c[k] * bit[k,j]) for all k")
    print("   b) Apply inverse confusion: c[j] = inv_confusion1[temp[j]]")
    print("   This reverses the Forward() operation which does confusion→diffusion")
    
    print("\n4. Mathematical Properties:")
    print("   - inv_confusion1 is a permutation matrix (sparse, one 1 per row/column)")
    print("   - inv_diffusion is a dense binary matrix")
    print("   - The product is NOT a simple matrix multiplication")
    print("   - Instead, it's a composition: confusion⁻¹ ∘ diffusion⁻¹")
    
    # Verify inverse is correct
    print("\n5. Verification:")
    # Multiply diffusion * diffusion_inv in GF(2)
    is_identity = True
    for i in range(32):
        result = 0
        for k in range(32):
            if (diffusion[i] >> k) & 1:
                result ^= diffusion_inv[k]
        if result != (1 << i):
            is_identity = False
            break
    
    print(f"   Diffusion * Diffusion_inv = Identity: {'✓ YES' if is_identity else '✗ NO'}")
    
    print("\n6. Usage in the cipher:")
    print("   - Forward: confusion → diffusion (repeated 256 times)")
    print("   - Backward: diffusion⁻¹ → confusion⁻¹ (repeated 256 times)")
    print("   - Final step: special XOR combination for compression")

if __name__ == "__main__":
    analyze_structure()